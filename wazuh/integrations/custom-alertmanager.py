...
def main():
    ...
    # SCA compliance findings (CIS, etc.): suppress real-time paging for level<12.
    # Compliance drift belongs in the daily digest, not Telegram.
    # Critical SCA findings (level>=12) still route normally; they remain visible
    # in the Wazuh dashboard regardless of this filter (audit trail preserved).
    # Added 2026-05-07.
    if level < 12 and any('sca' in str(g).lower() for g in groups):
        sys.exit(0)
