...
SED_RULES=(
    # Hostnames + private IPv4
    -e 's/\blocal\.codistech\.live\b/local.example.home/g'
    -e 's/\bcodistech\.live\b/example.home/g'
    -e 's/\b192\.168\.\([0-9]\+\)\.\([0-9]\+\)\b/10.0.\1.\2/g'
    # Vault tokens — modern (hvs.*) and legacy (s.*) prefixes
    -e 's/hvs\.[A-Za-z0-9_.-]\{10,\}/CHANGE_ME/g'
    -e 's/\bs\.[A-Za-z0-9]\{24,\}/CHANGE_ME/g'
    # YAML quoted-value secret fields — token / secret / password (20+ char)
    -e 's/\(token: *"\)[A-Za-z0-9+/=_.:-]\{20,\}"/\1CHANGE_ME"/g'
    -e "s/\\(token: *'\\)[A-Za-z0-9+/=_.:-]\\{20,\\}'/\\1CHANGE_ME'/g"
    -e 's/\(secret: *"\)[A-Za-z0-9+/=_.:-]\{20,\}"/\1CHANGE_ME"/g'
    -e "s/\\(secret: *'\\)[A-Za-z0-9+/=_.:-]\\{20,\\}'/\\1CHANGE_ME'/g"
    -e 's/\(password: *"\)[A-Za-z0-9+/=_.:-]\{20,\}"/\1CHANGE_ME"/g'
    -e "s/\\(password: *'\\)[A-Za-z0-9+/=_.:-]\\{20,\\}'/\\1CHANGE_ME'/g"
)
...
# --check mode.
LEAK_PATTERNS='(\bcodistech\.live\b|\b192\.168\.[0-9]+\.[0-9]+\b|\bhvs\.[A-Za-z0-9_.-]{10,}\b|\bs\.[A-Za-z0-9]{24,}\b)'
